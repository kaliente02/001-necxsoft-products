...
async function fetchBookingsFromDB() {
  try {
    const userEmail = localStorage.getItem('nexa_user_email');
    if (!userEmail) return [];
    
    // Filter bookings by current user's email only
...
async function submitBookingToDB(formData) {
  try {
    ...
    // Store user's email for future bookings view
...
function renderConfirmationView() {
  return `
    <div class="confirmation">
      ...
      <div class="actions">
        <button class="btn" id="btn-view-booking">View your bookings ${ICONS.arrowRight}</button>
...
async function init() {
  // Don't fetch all bookings on load anymore
  state.loading = false;
  render();
}
